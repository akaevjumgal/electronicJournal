...
def login():
    ...
    if request.method == 'POST':
        ...
        if user_info['code'] != 200:
            ...
            token = jwt.encode({'public_id': user_info['data'][0][1], 'exp': datetime.datetime.utcnow()+datetime.timedelta(seconds=100)}, app.config['SECRET_KEY'])
...
@token_required
def logged(current_user,public_Id):
    return 'Привет '